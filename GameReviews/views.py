...
def requestReviews(myTempString):
        ...
        for review in sortedReviews:
            ...
            for values in review:
                if values not in noPrint:
                    myTmpStr += str(values) + review[values] + '\n'
            myTmpStr += '\n'
        myTmpStr += '\n'
        return myTmpStr
    ...
    for urls in urlDict:
        ...
        if urls == 'destructoid':
            ...
            if len(greatReviews) > 0:
                anotherTmpSTRING += '-----Destructoid Reviews-----\n'
                anotherTmpSTRING += printReviews(greatReviews)
                anotherTmpSTRING += '\n'
        if urls == 'steam':
            greatReviews = []
            grabSteamReviews()
            if len(greatReviews) > 0:
                anotherTmpSTRING += '-----Steam Reviews-----\n'
                anotherTmpSTRING += printReviews(greatReviews)
                anotherTmpSTRING += '\n'
    return anotherTmpSTRING
# myTmpStr =''
# if len(greatReviews) > 0:
#     sortedReviews = sorted(greatReviews, key = lambda k: k['floatScore:'], reverse=True) #Sort by float value
#     for review in sortedReviews:
#         currScore = review['Score:']
#         noPrint = {'floatScore:','Publisher:', 'Reviewed On:'} #Valid tags, but not really things that matter to me 
#         for values in review:
